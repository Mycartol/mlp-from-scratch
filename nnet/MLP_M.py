...
x,y_t,x_norm,x_n_s,y_t_s = hkl.load('echocardiogram.hkl')
...
K1_vec = np.array([1,3,5,7,9,11,13,15])
K2_vec = K1_vec
...
# Wczytanie danych (jeśli jeszcze nie załadowane)
x, y_t, x_norm, x_n_s, y_t_s = hkl.load('echocardiogram.hkl')

# Utworzenie i trenowanie modelu z optymalnymi parametrami (załóżmy, że znamy już najlepsze parametry)
best_K1 = K1_vec[k1_ind_max]
best_K2 = K2_vec[k2_ind_max]
best_lr = lr_vec[lr_ind_max]
best_mc = mc_vec[mc_ind_max]

final_model = mlp_m_3w(x_norm, y_t, best_K1, best_K2, best_lr, err_goal, disp_freq, best_mc, max_epoch, True)
final_model.train(x_norm, y_t)  # Trening na wszystkich danych

# Predykcja na POSORTOWANYCH danych (x_n_s)
y_pred_sorted = final_model.predict(x_n_s)

# Wykres porównawczy
plt.figure(figsize=(12, 6))
plt.plot(y_t_s[0], 'b-', label='Dane rzeczywiste (posortowane)', linewidth=2, alpha=0.7)
plt.plot(y_pred_sorted[0], 'r--', label='Predykcje modelu', linewidth=2)
plt.xlabel('Indeks próbki (posortowane)', fontsize=12)
plt.ylabel('Wartość docelowa', fontsize=12)
plt.title('Porównanie danych rzeczywistych i predykcji modelu na danych posortowanych', fontsize=14)
